...
var Parser = (function (scope) {
			...
			for (i = 0; i < L; i++) {
				...
				if (type_ === TNUMBER) {
					...
					if (item.index_ in values) {
						nstack.push(parseFloat(values[item.index_]));
					}
	...
	function smaller_equal(a, b) {
		return a <= b;
	}
	
	function bigger_equal(a, b) {
		return a >= b;
	}
...
//\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
		...
		isOperator: function () {
			...
			if (code === 43) { // +
				...
				if (this.expression.charCodeAt(this.pos + 1) === 110) 
				{
					if (this.expression.charCodeAt(this.pos + 2) === 100) 
					{
						this.pos+=2;
						this.tokenprio = 0;
						this.tokenindex = "and";
